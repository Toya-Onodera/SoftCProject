...
public class SearchBook extends JFrame implements ActionListener {
	...
	// 入力モード切替
	private boolean inputModeFlg = false;
	...
	public void setMyLayout () {
		...
		addElement(inputButton, 3, 1, 1, 1);
	 ...
	 * 入力モードの切り替えを行う
	 */
	public void textFieldActiveChange () {
		inputModeFlg = !inputModeFlg;
	...
	public void textFieldEnable () {
		if (inputModeFlg) {
			// タイトルテキストフィールドにフォーカスを与える
			titleTextField.requestFocusInWindow();

			// 検索テキストフィールドのボーダー色を変更
			searchTextField.setBorder(new LineBorder(new Color(200, 200, 200), 1));

			// 選択不可テキストフィールドの背景色を変更
			searchTextField.setBackground(new Color(255, 255, 255));

			// 選択可テキストフィールドのボーダー色を変更
			titleTextField.setBorder(new LineBorder(new Color(3, 169, 244), 1));
			authorTextField.setBorder(new LineBorder(new Color(3, 169, 244), 1));
			publisherTextField.setBorder(new LineBorder(new Color(3, 169, 244), 1));
			yearTextField.setBorder(new LineBorder(new Color(3, 169, 244), 1));
			isbnTextField.setBorder(new LineBorder(new Color(3, 169, 244), 1));
			...
			// 選択不可テキストフィールドのボーダー色を変更
			titleTextField.setBorder(new LineBorder(new Color(200, 200, 200), 1));
			authorTextField.setBorder(new LineBorder(new Color(200, 200, 200), 1));
			publisherTextField.setBorder(new LineBorder(new Color(200, 200, 200), 1));
			yearTextField.setBorder(new LineBorder(new Color(200, 200, 200), 1));
			isbnTextField.setBorder(new LineBorder(new Color(200, 200, 200), 1));

		}

		searchTextField.setEditable(!inputModeFlg);
		titleTextField.setEditable(inputModeFlg);
		authorTextField.setEditable(inputModeFlg);
		publisherTextField.setEditable(inputModeFlg);
		yearTextField.setEditable(inputModeFlg);
		isbnTextField.setEditable(inputModeFlg);
	}
	...
	public void actionPerformed (ActionEvent e) {
		...
		// 保存ボタン
			...
			if (inputModeFlg == false) {
				// 保存が行える状態か
				if (result.size() > 0) {
					// 保存ができたかを入れる
					boolean isSave = _bookdb.saveResult(result);
				...
				Map<String, String> addData = new HashMap<String, String>() {
					{
						put("タイトル", titleTextField.getText());
						put("著者", authorTextField.getText());
						put("出版社", publisherTextField.getText());
						put("出版年", yearTextField.getText());
						put("ISBN", isbnTextField.getText());
					}
				};

				boolean isValue = true;
				for (Map.Entry<String, String> _m : addData.entrySet()) {
					if (_m.getValue().isEmpty()) {
						isValue = false;
						break;
					}
				}
				...
				if (isValue) {
					...
					JOptionPane.showMessageDialog(this, new JLabel("入力されていない項目があります。"));
				}
			}
		}

		else if (e.getSource() == inputButton) {
			_sb.textFieldActiveChange();
			this.result.clear();
			resultLabelRight.setText("0 / 0");
